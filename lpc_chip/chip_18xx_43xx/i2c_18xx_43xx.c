...
// 2019-7-23, event driven 방식으로 변경

extern void _signalSend(struct i2c_interface *iic, int flag);
extern int  _signalWait(int flag, int tout);

// irq 처리 부분에서 문제 발생
int Chip_I2C_EventHandler(I2C_ID_T id, I2C_EVENT_T event)
{
	...
	if (_eepWait(0x1, 50) < 0) {
		iic->ip->CONCLR = I2C_CON_SI | I2C_CON_STA | I2C_CON_AA; //I2C_CON_SI | I2C_CON_STA | I2C_CON_AA;
		return -1;
	}
#else	
	...
	while (*stat == I2C_STATUS_BUSY && retry-- > 0) {}
	if (retry == 0) {
		printf("Chip_I2C_EventHandler Timeout \n");
		return -1;
	}
	else {
		return 0;
...
// endless loop 보완
void *get_i2c_interface(int id) {
	return &i2c[id];
}

/* Chip polling event handler */
int Chip_I2C_EventHandlerPolling(I2C_ID_T id, I2C_EVENT_T event)
{
	struct i2c_interface *iic = &i2c[id];
	volatile I2C_STATUS_T *stat;
	int retry = 10000;
...
// system_lpc43xx.c를 사용시 0, sysinit.c 사용시 1로 한다   
#if 0
   clkrate = 180000000;
#else
   clkrate = getClkRate(id);   
#endif   
	SCLValue = (clkrate / clockrate);
...
int Chip_I2C_MasterTransfer(I2C_ID_T id, I2C_XFER_T *xfer)
{
	...
	while (!isI2CBusFree(iic->ip) && retry-- > 0) {}
		
	if (retry == 0) {
		printf("\t Wait for stop condition ...\n");
	}
