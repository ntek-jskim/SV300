...
#if 1 	// 수신장치 등록시 mask 사용할 수 있게 한다 
/* Set a message into the message object in message RAM */
void Chip_CCAN_SetMsgObjectWithMask(LPC_CCAN_T *pCCAN,
							CCAN_MSG_IF_T IFSel,
							...
							uint32_t mask)
{
	...
	if (!(pMsgObj->id & (0x1 << 30))) {					/* bit 30 is 0, standard frame */
		...
		pCCAN->IF[IFSel].MSK2 = CCAN_IF_MASK2_MXTD | CCAN_IF_MASK2_MDIR(dir) | ((CCAN_MSG_ID_EXT_MASK >> 16) & (mask >> 16));
		pCCAN->IF[IFSel].MSK1 = CCAN_MSG_ID_EXT_MASK & 0x0000FFFF & mask;
...
#if 1	// 수신용 장치용 Object 할당하고, CAN id 등록시 mask  사용할 수 있게 한다
/* Register a message ID for receiving */
void Chip_CCAN_AddReceiveIdWithMask(LPC_CCAN_T *pCCAN, CCAN_MSG_IF_T IFSel, uint32_t id, uint32_t mask)
{
	CCAN_MSG_OBJ_T temp;
	uint8_t msgNum = getFreeMsgObject(pCCAN);
	if (!msgNum) {
	...
	temp.id = id;
	Chip_CCAN_SetMsgObjectWithMask(pCCAN, IFSel, CCAN_RX_DIR, false, msgNum, &temp, mask);
}
#endif
