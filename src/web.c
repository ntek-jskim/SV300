...
" h+='</div>';$('setBody').innerHTML=h;$('setNote').textContent=IS_ADMIN?'변경 후 Save & Apply 하면 설정 저장을 위해 장치가 재부팅됩니다(재접속 후 재로그인).':'Viewer 모드 - 읽기 전용.';});}\n"
...
"function afterSave(nn){setRes('Saved ('+nn+') - rebooting');\n"
" $('setBody').innerHTML=\"<p class='stub'>설정 저장 완료. 적용을 위해 장치가 재부팅됩니다. 재접속되면 자동으로 로그인 화면으로 전환됩니다...</p>\";$('setNote').textContent='';\n"
" var tr=0;setTimeout(function pl(){tr++;fetch('/api/me',{cache:'no-store'}).then(function(){location.reload();}).catch(function(){if(tr<40)setTimeout(pl,2000);else location.reload();});},8000);}\n"
...
" if(!chg.length){setRes('No changes');return;}if(!confirm(chg.length+'개 설정을 저장합니다. 적용을 위해 장치가 재부팅됩니다. 계속?'))return;setRes('Writing...');\n"
...
" function next(){if(i>=chg.length){j('/api/savecfg',{method:'POST'}).then(function(){afterSave(chg.length);});return;}\n"
